...
// 			}
		...
		onLoad(params) {
			...
			uni.request({
				url: serverUrl + '/search/trailer/' + trailerId + '?qq=1105254074',
				method: "POST",
				success: (res) => {
			...
			// 获取获取演员
			uni.request({
				url: serverUrl + '/search/staff/' + trailerId + '/2' + '&qq=1105254074',
				method: "POST",
				success: (res) => {
					// 获取真实数据之前，务必判断状态是否为200
					if (res.data.status == 200) {
		...
		onNavigationBarButtonTap(e) {
			...
			if (index == 0) {
				uni.share({
					...
					type: 0,
					href: "http://www.imovietrailer.com/#/pages/movie/movie?trailerId=" + trailerId + '&qq=1105254074',
